...
class Sigmoid:
    ...
    def activate(self, values):
        ...
        result = self.sigmoidLogistic(strength)
    ...
    def update(self, values, train, eta=.1):
        ...
        for X, y_true in zip(values, train):
            ...
            for j, val in enumerate(X):
                print "j, val: ", j, val
                self.weights[j] += eta * (y_true-y_pred)*dev*val
...
def test():
    ...
    def sum_almost_equal(array1, array2, tol = 1e-5):
        print "****************"
        print "tol", tol, tol<0.1, 1e-1, 1e-3
        print "sum", sum(abs(array1 - array2))
        return sum(abs(array1 - array2)) < tol

    u1 = Sigmoid(weights=[3,-2,1])
    print abs(u1.activate(np.array([1,2,3])) - 0.880797)
